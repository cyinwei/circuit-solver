...
classdef Circuit < handle
    ...
    methods
        ...
        function DrawTraverse(obj, curNode, traversed, visited, compV)
            disp(curNode.id);
                ...
                for i=1:numel(curNode.connects)
                    ...
                    if ~(strcmp(curNode.connects(i).cId,traversed))
                        ...
                        if ~(ismember(curNode.connects(i).adjNode,visited)) || (curNode.connects(i).adjNode==visited(1))
                            obj.DrawTraverse(obj.GetNode(curNode.connects(i).adjNode), curNode.connects(i).cId, visited, compV);
                        end
                    end
                end
            else
        ...
        function DrawTraverse2(obj, curNode, traversed, visited, compV, masterLoop)
            disp('traverse');
            disp(curNode.id);
            if ~strcmp(traversed,'none')
               compV{end+1} = traversed;
            ...
            if ~(ismember(curNode.id,masterLoop)) || numel(visited)==1
                for i=1:numel(curNode.connects)
                    ...
                    if ~(strcmp(curNode.connects(i).cId,traversed)) && ~(obj.GetComp(curNode.connects(i).cId).inLoop)
                        ...
                        disp(curNode.connects(i).cId);
                        if ~(ismember(curNode.connects(i).adjNode,visited)) || curNode.connects(i).adjNode==visited(1)
                            obj.DrawTraverse2(obj.GetNode(curNode.connects(i).adjNode), curNode.connects(i).cId, visited, compV, masterLoop);
                        end
                    end
                end
            else
...
>>>>>>> 29230a4034405979bba4ca6d832f5b58be1b585a:Code/Circuit.m
