...
classdef Circuit < handle
    ...
    methods
        ...
        function AddCSource(obj, c, n1, n2, id)
            if n1 ~= n2 %Ensure that n1 is not equal to n2 (that would be an error)
                ...
                if obj.AddNode(n1) %If n1 does not exist (ie- the AddNode function added a new node)
                    ...
                    for i = 1:numel(obj.nodes)
                        if obj.nodes(i).id==n1
                            obj.nodes(i).connects(end+1)=Identifier('c',numel(obj.csources), true,n2, id); %Add identifier to n1 to identify new csource connected to n1
                            break;
                        end
                    end
                end
        ...
        function AddShort(obj,n1,n2)
            sID=strcat('S',num2str(obj.curShort));
            obj.curShort=obj.curShort+1;
            obj.AddResistor(obj, 0, n1, n2, sID)
        end
        ...
        function comp = GetComp(obj, n)
           ...
           if tempT=='V'
               for i=1:numel(obj.vsources)
                   %disp(n);
                   %disp(obj.vsources(i).id);
                   if strcmp(obj.vsources(i).id,n)>0
                       comp=obj.vsources(i);
                       break
                   end
               end
           elseif tempT=='C'
               for i=1:numel(obj.csources)
                   if strcmp(obj.csources(i).id,n)>0
                       comp=obj.csources(i);
                       break
                   end
               end
           elseif tempT=='R' || tempT=='S'
               for i=1:numel(obj.resistors)
                   if strcmp(obj.resistors(i).id,n)>0
                       comp=obj.resistors(i);
                       break
                   end
               end
           end
        ...
        function ValueCSources(obj)
            for i=1:numel(obj.csources)
                obj.csources(i).voltage = obj.GetNode(obj.csources(i).node2).voltage - obj.GetNode(obj.csources(i).node1).voltage;
        ...
        function StartDrawTraverse(obj)
            ...
            if found %If a grounded node was found
               ...
               % else
                    %obj.grid=Grid(obj.drawLoops,obj.drawCompLoops,obj.drawOrients,obj);
                %end
        ...
        function VCurrents(obj)
           for i=1:numel(obj.nodes)
              currents=[];
              indSolve=-1;
              bail=false;
              numV=0;
              for j=1:numel(obj.nodes(i).connects)
                 tempC = obj.nodes(i).connects(j);
                 if tempC.type=='r'
                     if obj.GetNode(tempC.adjNode).voltage > obj.nodes(i).voltage %Current coming in
                         currents(end+1)=-(abs(obj.GetNode(tempC.adjNode).voltage-obj.nodes(i).voltage)/obj.resistors(tempC.index).resistance);
                     else %current going out
                         currents(end+1)=(abs(obj.GetNode(tempC.adjNode).voltage-obj.nodes(i).voltage)/obj.resistors(tempC.index).resistance);
                     end
                 elseif tempC.type=='c'
                     if tempC.positive
              ...
              if indSolve>-1 && ~bail
                  ...
                  for k=1:numel(currents)
                     theCur=theCur+currents(k); 
                  end
                  obj.nodes(i).connects(indSolve).vCurrent=theCur;
                  %disp('found current');
                  %disp(obj.nodes(i).id);
                  %disp(theCur);
                  %disp(obj.nodes(i).connects(indSolve).vCurrent);
              end
           end
            
           for i=1:numel(obj.vsources)
               tempN=obj.GetNode(obj.vsources(i).node1);
               for j=1:numel(tempN.connects)
                   if (strcmp(tempN.connects(j).cId,obj.vsources(i).id)>0) & (tempN.connects(j).vCurrent~=inf)
                       if tempN.connects(j).positive
                          obj.vsources(i).current=tempN.connects(j).vCurrent;
                       else
                          obj.vsources(i).current=-tempN.connects(j).vCurrent;
                       end
                   end
               end
               tempN2=obj.GetNode(obj.vsources(i).node2);
               for j=1:numel(tempN2.connects)
                   if (strcmp(tempN2.connects(j).cId,obj.vsources(i).id)>0) & (tempN2.connects(j).vCurrent~=inf)
                       if tempN2.connects(j).positive
                          %disp(obj.vsources(i));
                          obj.vsources(i).current=tempN2.connects(j).vCurrent;
                       else
                          obj.vsources(i).current=-tempN2.connects(j).vCurrent;
                       end
                   end
               end
               obj.vsources(i).power=-(obj.vsources(i).current*obj.vsources(i).voltage);
           end
        end
        ...
        function good = CheckCircuit(obj)
            ...
            for j=1:numel(obj.drawCompLoops{1})
                if obj.drawCompLoops{1}{j}(1)=='R' || obj.drawCompLoops{1}{j}(1)=='C'
                    ...
                    %disp(strcat('+',num2str(obj.GetNode(obj.drawLoops{1}(j)).voltage - obj.GetNode(obj.drawLoops{1}(j+1)).voltage)));
                elseif obj.drawCompLoops{1}{j}(1)=='V'
                    if obj.drawOrients{1}(j)==1
                        volts=volts-obj.GetComp(obj.drawCompLoops{1}{j}).voltage;
                        %disp(strcat('-',num2str(obj.GetComp(obj.drawCompLoops{1}{j}).voltage)));
                    else
                        volts=volts+obj.GetComp(obj.drawCompLoops{1}{j}).voltage;
                       %disp(strcat('+',num2str(obj.GetComp(obj.drawCompLoops{1}{j}).voltage)));
                    end
            ...
            if floor(abs(volts))~=0
                good=false;
            end
            
            for k=2:numel(obj.drawCompLoops)
                for j=1:numel(obj.drawCompLoops{k})
                    if obj.drawCompLoops{k}{j}(1)=='V'
                       if abs(obj.GetNode(obj.drawLoops{k}(j)).voltage) > abs(obj.GetNode(obj.drawLoops{k}(j+1)).voltage)
                           v1=obj.GetNode(obj.drawLoops{k}(j)).voltage;
                           v2=obj.GetNode(obj.drawLoops{k}(j+1)).voltage;
                           if floor((v1-v2)-abs(obj.GetComp(obj.drawCompLoops{k}{j}).voltage))~=0
                               good=false;
                           end
                       else
                           v1=obj.GetNode(obj.drawLoops{k}(j)).voltage;
                           v2=obj.GetNode(obj.drawLoops{k}(j+1)).voltage;
                           if floor((v2-v1)-abs(obj.GetComp(obj.drawCompLoops{k}{j}).voltage))~=0
                               good=false;
                           end
                       end
                    end
        ...
        function loopComp = BiggestLoop(obj)
            if numel(obj.loops)>0
                ...
                tempCon=[];
                for k=1:numel(obj.loops{ind})
                    tempCon(end+1) = numel(obj.GetNode(obj.loops{ind}(k)).connects);
                end
                obj.nodeCon{end+1}=tempCon;
                obj.loops=[];
                obj.orients=[];
        ...
        function DrawTraverse(obj, curNode, traversed, visited, compV, dirs)
            ...
            if ~strcmp(traversed,'none')
                ...
                %disp(curNode.id);
                if traversed(1)=='R'
                    if (curNode.voltage > obj.GetNode(visited(end)).voltage)
                        dirs(end+1) = 1;
                        %disp('if1');
                        %disp(curNode.voltage);
                        %disp('bk1');
                        %disp(obj.GetNode(visited(end)).voltage);
                    else
                        dirs(end+1) = 0;
                        %disp('if2');
                        %disp(curNode.voltage);
                ...
                elseif tempComp.node1 == curNode.id
